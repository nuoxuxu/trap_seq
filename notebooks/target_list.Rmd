...
## IP only target list
```{r}
genes_of_interest_IP <- read.csv("results/IP_res.csv") %>%
    dplyr::filter(padj < 0.05) %>% 
    pull(X)  

temp <- lapply(GO_terms, function(x) {genes_of_interest_IP %in% x}) %>% data.frame()
...
temp <- lapply(GO_terms, function(x) {genes_of_interest_RE %in% x}) %>% data.frame()
